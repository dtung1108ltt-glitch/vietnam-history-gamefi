...
namespace VnHistoryGameFi.Network
{
    ...
    /// mọi request đi qua network thật, backend thật sẽ tự gọi RPC Sui/Solana
    /// thật để xác minh (xem backend/app/blockchain/sui_adapter.py và
    /// solana_adapter.py) — Unity ở đây không giả lập gì cả.
    ///
    ...
    public class ApiBlockchainAdapter : MonoBehaviour, IBlockchainAdapter
    {
            ...
            Action<PlayerDto> onSuccess, Action<string> onError)
        {
            if (string.IsNullOrEmpty(signature))
            {
                ...
                onError?.Invoke(
                    ...
                    "signature — cần tích hợp một wallet SDK/plugin thật (Sui " +
                    "hoặc Solana) để lấy signature trước khi gọi VerifyWallet().");
                return;
            }
            ...
            _client.Post<PlayerDto>("/auth/wallet", body, onSuccess, onError);
        }
        ...
        public void ClaimReward(string wallet, int battleId,
            Action<RewardDto> onSuccess, Action<string> onError)
        {
            var body = new RewardClaimRequestDto { wallet = wallet, battle_id = battleId, amount = 0 };
