...
bằng chứng giao dịch và quyền sở hữu để xác minh với SuiAdapter/SolanaAdapter.
"""
...
class Store:
    ...
    def _seed_default_data(self) -> None:
        """Khởi tạo một số listing mẫu trên chợ tướng để test."""
        sample_listings = [
            MarketplaceListing(
                listing_id="list-tran-hung-dao-01",
                advisor_id="tran_hung_dao",
                seller_wallet="0x8f2a1b94c3d8e5f67a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f",
                price=25.0,
                currency="SUI",
                chain="sui",
                token_id="0xnft_thd_legendary_01",
                status="active",
            ),
            MarketplaceListing(
                listing_id="list-quang-trung-01",
                advisor_id="quang_trung",
                seller_wallet="HN7cABqLq46Es1jh92dQQisAq662SmxELLLsHHe4YWrH",
                price=30.0,
                currency="SOL",
                chain="solana",
                token_id="sol_nft_qt_01",
                status="active",
            ),
            MarketplaceListing(
                listing_id="list-ngo-quyen-01",
                advisor_id="ngo_quyen",
                seller_wallet="0x1a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2b",
                price=18.5,
                currency="SUI",
                chain="sui",
                token_id="0xnft_nq_01",
                status="active",
            ),
        ]
        for l in sample_listings:
            self.listings[l.listing_id] = l
            self.advisor_ownerships[f"{l.chain}:{l.token_id}"] = AdvisorOwnership(
                advisor_id=l.advisor_id,
                owner_wallet=l.seller_wallet,
                chain=l.chain,
                token_id=l.token_id,
            )
    ...
    def create_guest_player(self, username: str | None = None) -> Player:
        ...
        chain = "sui"  # default chain designation for guest session
        key = f"guest:{wallet}"
        player = Player(
            wallet=wallet,
            chain=chain,
