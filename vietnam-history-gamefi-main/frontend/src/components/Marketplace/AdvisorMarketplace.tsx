...
export const AdvisorMarketplace: React.FC<AdvisorMarketplaceProps> = ({
  ...
  const [listTokenId, setListTokenId] = useState<string>('0xnft_adv_sample');
  ...
  const handleBuy = async (listing: MarketplaceListing) => {
    if (!player || player.is_guest) {
      setStatusMessage('Vui lòng kết nối ví blockchain (Sui hoặc Solana) để thực hiện giao dịch mua.');
      onOpenWalletModal();
      return;
    }

    onPlaySword();
    setStatusMessage(`Đang xác nhận chữ ký ví mua ${listing.advisor_name}...`);
    try {
      const mockDigest = `tx_buy_${Math.random().toString(36).substring(2, 10)}`;
      await apiService.buyListing({
        listing_id: listing.listing_id,
        buyer_wallet: player.wallet,
        tx_digest: mockDigest,
      });
      setStatusMessage(`Giao dịch thành công! Bạn đã sở hữu Tướng Cố Vấn ${listing.advisor_name} trên mạng ${listing.chain.toUpperCase()}.`);
      loadData();
    } catch (e) {
      setStatusMessage('Giao dịch mua thất bại. Vui lòng thử lại.');
    }
  };
  ...
  const handleCreateListing = async (e: React.FormEvent) => {
    ...
    onPlayDrum();
    setStatusMessage('Đang khởi tạo niêm yết trên hợp đồng thông minh...');
    try {
      await apiService.createListing({
        advisor_id: listAdvisorId,
        seller_wallet: player.wallet,
        chain: player.chain || 'sui',
        price: parseFloat(listPrice) || 10.0,
        currency: player.chain === 'solana' ? 'SOL' : 'SUI',
        token_id: listTokenId,
      });
      setStatusMessage('Đã niêm yết Tướng Cố Vấn thành công lên Chợ!');
      setActiveTab('browse');
      loadData();
    } catch (e) {
      setStatusMessage('Không thể tạo niêm yết. Vui lòng kiểm tra lại thông tin.');
    }
  };
  ...
  const handleCreateTrade = async (e: React.FormEvent) => {
    ...
    onPlayDrum();
    setStatusMessage('Đang gửi lời đề nghị trao đổi P2P...');
    try {
      const t = await apiService.createTrade({
        initiator_wallet: player.wallet,
        target_wallet: tradeTargetWallet || '0xTargetPeerPlayerWallet',
        offered_advisor_id: tradeOfferedId,
        requested_advisor_id: tradeRequestedId,
        chain: player.chain || 'sui',
      });
      setTrades(prev => [t, ...prev]);
      setStatusMessage('Đã gửi đề nghị trao đổi thành công! Đang đợi đối phương phê duyệt chữ ký ví.');
    } catch (e) {
      setStatusMessage('Không thể tạo đề nghị trao đổi.');
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center space-x-2 text-amber-400 text-xs font-bold uppercase tracking-wider mb-1">
              ...
              <span>Lớp Giao Dịch Sở Hữu On-Chain (Sui & Solana)</span>
            </div>
      ...
      {activeTab === 'browse' && (
        <div>
          ...
          {/* Listings Grid */}
          {loading ? (
            <div className="text-center py-16 text-slate-400">Đang đồng bộ dữ liệu niêm yết từ Sui & Solana...</div>
          ) : listings.length === 0 ? (
      ...
      {activeTab === 'list' && (
        <div className="max-w-2xl mx-auto bg-imperial-lacquer/80 border border-imperial-border rounded-2xl p-6 sm:p-8">
          ...
          <form onSubmit={handleCreateListing} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-300 mb-1">Giá Bán (SUI hoặc SOL):</label>
              <input
                type="number"
                step="0.1"
                min="0.1"
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 rounded-xl bg-imperial-obsidian border border-slate-700 text-white text-xs"
                placeholder="0x9a8b... hoặc PDA Solana"
                required
              />
            </div>
            ...
            <form onSubmit={handleCreateTrade} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="0x... hoặc địa chỉ ví Solana"
                  required
                />
              </div>
