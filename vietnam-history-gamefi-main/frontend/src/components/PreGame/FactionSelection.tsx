...
import { Sparkles, Shield, Swords, Zap, ArrowLeft, ArrowRight, Loader2, CheckCircle, Award, Crown, Play } from 'lucide-react';
...
export const FactionSelection: React.FC<FactionSelectionProps> = ({
  ...
  const handleSelectF2P = async () => {
    ...
    try {
      if (player?.wallet) {
        const updated = await apiService.selectFactionF2P(player.wallet, selectedFaction.faction_id);
          ...
          <div className="bg-black/50 border border-imperial-border rounded-xl p-5 flex flex-col items-center justify-center text-center space-y-4">
            ...
            {mintStatus && (
              <div className="w-full p-2 rounded-lg bg-amber-950/50 border border-amber-500/50 text-xs text-amber-200 flex items-center justify-center space-x-2">
                {isMinting ? <Loader2 className="w-3.5 h-3.5 animate-spin text-amber-400" /> : <CheckCircle className="w-3.5 h-3.5 text-emerald-400" />}
            ...
            <div className="w-full space-y-2">
              ...
              >
                ...
                <span>{isAlreadyOwned ? 'Tiếp Tục Vào Sảnh Duyệt Binh' : 'Chọn Triều Đại & Xuất Binh (F2P)'}</span>
                <ArrowRight className="w-4 h-4" />
              </button>

              {/* Optional: Mint on Blockchain if wallet connected */}
              {player && !player.is_guest && (
                <button
                  onClick={handleMint}
                  disabled={isMinting || isAlreadyOwned}
                  className="w-full py-2 px-3 rounded-lg bg-imperial-lacquer/80 hover:bg-slate-800 border border-slate-700 text-slate-300 hover:text-white text-xs font-semibold flex items-center justify-center space-x-1.5 transition-all cursor-pointer disabled:opacity-50"
                >
                  <Sparkles className="w-3.5 h-3.5 text-imperial-gold" />
                  <span>{isAlreadyOwned ? 'Đã Khắc Dấu On-Chain' : 'Khắc Dấu Faction NFT (Tùy Chọn)'}</span>
                </button>
              )}
            </div>
