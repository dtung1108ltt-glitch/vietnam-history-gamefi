...
export const DefiHub: React.FC<DefiHubProps> = ({ player, onBack, onPlayDrum }) => {
  ...
  const simulateAction = (label: string) => {
    onPlayDrum();
    const digest = `0x${Math.random().toString(16).slice(2, 10)}…${Math.random().toString(16).slice(2, 6)}`;
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 bg-imperial-lacquer/90 border border-imperial-border rounded-2xl p-6">
          ...
          {module === 'dex' && (
            <div>
              <label className="block text-[11px] text-slate-400 mb-1">Bán (SUI)</label>
              <input
                value={swapFrom}
              ...
              <p className="text-[11px] text-slate-400 mb-4">Giá pool 1 SUI = 1.84 USDC • Slippage tối đa 0.50% • Impact ước tính thấp</p>
              <button
                onClick={() => simulateAction(`Swap ${swapFrom} SUI → ${estimatedReceive} USDC`)}
            ...
            <div>
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div className="bg-black/30 border border-imperial-gold/30 rounded-xl p-4">
                  ...
                  <div className="text-2xl font-mono text-imperial-lightgold">48,760 SUI</div>
                </div>
                <div className="bg-black/30 border border-slate-800 rounded-xl p-4">
                  ...
                  <div className="text-2xl font-mono text-amber-200">−1,080 SUI</div>
                </div>
              </div>
