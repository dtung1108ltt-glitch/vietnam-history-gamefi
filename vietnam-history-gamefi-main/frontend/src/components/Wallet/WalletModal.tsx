      ...
      >
        ...
        <div className="mb-6">
          ...
          <p className="text-xs text-slate-400 mt-1">
            Theo kiến trúc MVP: Ví của bạn nắm giữ quyền sở hữu Faction NFT và bảo chứng thành tích on-chain.
          </p>
        </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className={`p-3 rounded-xl border flex flex-col items-center justify-center transition-all ${
                selectedChain === 'sui'
                  ? 'bg-blue-950/50 border-cyan-400 shadow-md shadow-cyan-950/50 scale-[1.02]'
                  : 'bg-black/40 border-imperial-border hover:border-slate-600 text-slate-400'
              }`}
            >
              <div className="w-8 h-8 rounded-full bg-cyan-600/30 flex items-center justify-center text-cyan-300 font-bold mb-1">
                💧
              </div>
              <span className="font-semibold text-sm text-slate-100">SUI Network</span>
              <span className="text-[10px] text-cyan-400/80">Sui Move &bull; Testnet</span>
            </button>
        ...
        <div className="bg-black/40 border border-slate-800 rounded-xl p-3 mb-5 text-xs space-y-1.5 text-slate-300">
          ...
          <ul className="text-[11px] text-slate-400 space-y-1 pl-4 list-disc">
            ...
            <li>Tự động nhận diện Ví trình duyệt hoặc tạo ví Sandbox để thử nghiệm ngay.</li>
          </ul>
        </div>
        ...
        >
          {isConnecting ? (
            ...
            <>
              <span>Xác Nhận Kết Nối {selectedChain === 'sui' ? 'Sui' : 'Solana'}</span>
        ...
        <p className="text-center text-[11px] text-slate-500 mt-3">
          Hỗ trợ Sui Wallet, Suiet, Phantom, Solflare & Sandbox Test Mode.
        </p>
      </div>
    </div>
  );
