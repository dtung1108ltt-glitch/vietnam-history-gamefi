...
import { suiAdapter } from '../services/sui';
...
export function useFaction(player: Player | null, onFactionRegistered: (factionId: number, nftId: string) => void) {
  ...
  const mintFactionNft = useCallback(async (factionId: number) => {
    ...
    try {
      const adapter = player.chain === 'sui' ? suiAdapter : solanaAdapter;
      const { tx_digest, nft_object_id } = await adapter.mintFactionNft(factionId);
