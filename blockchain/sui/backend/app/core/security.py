"""Nonce + xác thực chữ ký SUI personal message (flow Wallet login)."""
...
class NonceStore:
    ...
    def create(self, wallet: str) -> tuple[str, str]:
        nonce = secrets.token_hex(16)
        self._nonces[wallet.lower()] = (nonce, time.time() + self.ttl)
        return nonce, LOGIN_MESSAGE_TEMPLATE.format(nonce=nonce)

    def consume(self, wallet: str, nonce: str) -> bool:
        entry = self._nonces.pop(wallet.lower(), None)
        if entry is None:
            return False
